...
function renderMarktKies() {
  ...
  var morgenStart = new Date(nu.getFullYear(), nu.getMonth(), nu.getDate() + 1, 0, 0, 0);
  var morgenEind  = new Date(nu.getFullYear(), nu.getMonth(), nu.getDate() + 1, 23, 59, 59);
  var eersteVrije = null;

  for (var mi = 0; mi < weekLijst.length; mi++) {
    var mDat = weekLijst[mi].marktDatum;
    if (mDat >= morgenStart && mDat <= morgenEind) {
      eersteVrije = weekLijst[mi].id;
      break;
    }
  }

  if (!eersteVrije && weekLijst.length > 0) {
  
    var gesorteerd = weekLijst.slice().sort(function(a,b){ return a.marktDatum - b.marktDatum; });
    for (var mi2 = 0; mi2 < gesorteerd.length; mi2++) {
      if (gesorteerd[mi2].marktDatum > morgenEind) {
        eersteVrije = gesorteerd[mi2].id;
        break;
      }
    }
  }
